...
class Frame(object):
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError
        for variable in formals:
            frame.bindings[variable] = vals.first
            vals = vals.second
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        body = vals.second
        env.bindings[func_name] = do_lambda_form(Pair(formals, body), env)
        return func_name
    else:
...
def do_let_form(vals, env):
    ...
    while bindings is not nil:
        names, values = Pair(bindings.first.first, names), Pair(scheme_eval(bindings.first.second.first, env), values)
...
def do_or_form(vals, env):
    ...
    n, i = len(vals), 0
    while i < n:
        if scheme_eval(vals[i], env) is True:
            return True
        elif scheme_eval(vals[i], env) is not False:
            return vals[i]
        i += 1
    return vals[n - 1]
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if clause.first == "else":
            ...
            test = clause.first
        if scheme_true(scheme_eval(test, env)):
...
def check_formals(formals):
    ...
    """
    _check_formals_helper(formals)

def _check_formals_helper(formals, current=[]):
    if formals == nil:
        return
    if not scheme_symbolp(formals.first):
        raise SchemeError(str(formals.first) + " is not a valid list.")
    if formals.first in current:
        raise SchemeError(str(formals.first) + " has already been used as a symbol.")
    elif formals.second != nil:
        if type(formals.second) != Pair:
            raise SchemeError("Ill formed list")
        _check_formals_helper(formals.second, current + [formals.first])
